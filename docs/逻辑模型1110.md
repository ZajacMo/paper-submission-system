...
| `update_date` | `DATETIME` | `NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP` | 更新日期 |
...
| `status` | `ENUM('Accept', 'Reject', 'Major Review', 'Minor Review')` | `NULL` | 论文状态 |
| `status_read` | `BOOLEAN` | `NOT NULL DEFAULT FALSE` | 状态是否已读 |
...
| `page_number` | `VARCHAR(50)` | `NOT NULL` | 页码 |
...
#### 说明：
本逻辑模型基于API文档进行了修正，以确保数据库表结构与实际API接口的数据字段要求一致。主要修正内容包括：

1. **用户认证相关字段**：为authors、experts、editors表添加了id和role字段，用于JWT认证
2. **论文状态管理**：为papers表添加了status、status_read字段，扩展了progress枚举值
3. **状态追踪字段**：为review_assignments和notifications表添加了is_read字段
4. **字段兼容性优化**：为keywords和funds表添加了API兼容字段
5. **字段约束调整**：调整了部分字段的默认值和可空性
...
| `bio` | `TEXT` | `NULL` | 个人简介 |
| `phone` | `VARCHAR(20)` | `NULL` | 手机 |
| `role` | `VARCHAR(20)` | `NOT NULL DEFAULT 'author'` | 用户角色
...
| `account_holder` | `VARCHAR(100)` | `NULL` | 开户名 |
| `review_fee` | `DECIMAL(10, 2)` | `NOT NULL` | 审稿费 |
| `role` | `VARCHAR(20)` | `NOT NULL DEFAULT 'expert'` | 用户角色
...
| `email` | `VARCHAR(100)` | `NOT NULL, UNIQUE` | 邮箱 |
| `password` | `VARCHAR(255)` | `NOT NULL` | 密码|
| `role` | `VARCHAR(20)` | `NOT NULL DEFAULT 'editor'` | 用户角色
...
| `attachment_path` | `VARCHAR(500)` | `NOT NULL` | 论文附件存储路径 |
| `submission_date` | `DATETIME` | `NOT NULL DEFAULT CURRENT_TIMESTAMP` | 投稿日期 |
| `submit_time` | `DATETIME` | `NOT NULL DEFAULT CURRENT_TIMESTAMP` | 提交时间（与submission_date同步） |
| `progress` | `ENUM('Processing','Finished','under_review')` | `NOT NULL DEFAULT 'Processing'` | 进度描述 |
| `integrity` | `ENUM('True', 'False', 'Waiting')` | `NOT NULL DEFAULT 'Waiting'` | 完整性 |
| `check_time` | `DATETIME` | `NULL` | 检查时间 |
| `status` | `ENUM('submitted','Accept','Minor Revision','Major Revision','Reject')` | `NOT NULL DEFAULT 'submitted'` | 论文状态 |
| `status_read` | `BOOLEAN` | `NOT NULL DEFAULT FALSE` | 状态是否已读
...
| :--- | :--- | :--- | :--- |
| `keyword_id` | `INT` | `PRIMARY KEY, AUTO_INCREMENT` | 关键词唯一ID |
| `keyword_name` | `VARCHAR(50)` | `NOT NULL` | 关键词名称（扩展长度以支持更多内容） |
| `keyword_type` | `ENUM('zh', 'en')` | `NOT NULL` | 关键词类型（zh-中文关键词，en-英文关键词） |
| `name` | `VARCHAR(50)` | `NOT NULL` | 关键词名称（与keyword_name字段保持一致，用于API兼容） |
| `type` | `ENUM('zh', 'en')` | `NOT NULL` | 关键词类型（与keyword_type字段保持一致，用于API兼容） |
...
| `project_name` | `VARCHAR(200)` | `NOT NULL` | 项目名称 |
| `project_number` | `VARCHAR(100)` | `NOT NULL` | 项目编号 |
| `name` | `VARCHAR(200)` | `NOT NULL` | 项目名称（与project_name字段保持一致，用于API兼容） |
| `number` | `VARCHAR(100)` | `NOT NULL` | 项目编号（与project_number字段保持一致，用于API兼容） |
...
| `expert_id` | `INT` | `FOREIGN KEY (experts.expert_id)` | 审稿专家ID |
| `editor_id` | `INT` | `FOREIGN KEY (editors.editor_id)` | 编辑ID |
| `conclusion` | `ENUM('Accept', 'Minor Revision', 'Major Revision', 'Not Reviewed', 'Reject')` | `NOT NULL DEFAULT 'Not Reviewed'` | 评审结论 |
| `positive_comments` | `TEXT` | `NULL` | 正面评价 |
| `negative_comments` | `TEXT` | `NULL` | 负面评价 |
| `modification_advice` | `TEXT` | `NULL` | 修改意见 |
| `status` | `ENUM('Assigned', 'Completed', 'Overdue')` | `NOT NULL DEFAULT 'Assigned'` | 任务状态 |
| `assigned_date` | `DATETIME` | `NOT NULL DEFAULT CURRENT_TIMESTAMP` | 审稿分配日期 |
| `assigned_due_date` | `DATETIME` | `NOT NULL` | 审稿截止日期 |
| `submission_date` | `DATETIME` | `NULL` | 意见提交日期 |
...
>>>>>>>> 3e0c954864f41bca43b6f147d2ec5b84268e4787:docs/逻辑模型1110.md
